...
第1の制約を流量保存則と呼び，第一項は頂点 i から出る流量，第二項は頂点 i に入る流量を表します．
第2の制約を容量制約と呼びます．
...
## 4. 初期 spanning tree structure の構築

初期 spanning tree structure (T, L, U) を構築します．  
...
$T$ に追加する辺（entering arc）を $U$ か $L$ から選びます．  
...
- Best eligible arc pivot rule(Dantzig's pivot rule)
  ...
  - violation の最も大きい eligible arc を entering arc とする
  - 1 flow 単位の改善が最も大きいため，イテレーションの回数は少なくなる
  - すべての non-tree arc を調べる必要があるため，1 回のイテレーションのコストが大きい

- First eligible arc pivot rule

  - 最初に見つけた eligible arc を entering arc とする．次のイテレーションでは，前回選択した辺の次から探索を開始する．最後の辺まで探索をしたら先頭に戻る
  - 1 回のイテレーションのコストが小さい
  - 1 flow 単位の改善が小さくなるので，イテレーションの回数が多くなる

- Block search pivot rule
  - 辺をブロックに分割し，ブロックの中で violation の最も大きい eligible arc を entering arc とする．ブロック内で見つからなかった場合，次のブロックを探索する
...
アルゴリズムの各イテレーションでは，全域木に辺を追加することで閉路を作り，この閉路の flow を更新することで目的関数値を減少させます．  
...
実は primal network simplex 法は entering arc と leaving arc を任意に選ぶと degenerate iteration が無限に発生しアルゴリズムが有限回で終了しないことがあります．
...
strongly feasible spanning tree は flow が下限の辺は根に向いていて，flow が上限の辺は根と反対を向いている全域木とみなすこともできます．
...
辺 (k, l) を entering arc とします．頂点 k と l の最小共通祖先を頂点 w とします．  
...
cycle free solution は辺を適切に選ぶことで spanning tree solution を求めることができます．  
